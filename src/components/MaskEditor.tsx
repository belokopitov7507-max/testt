          ...
          <section className="rounded-xl border border-ink-700 bg-ink-850/80 p-4">
            ...
            <div className="flex flex-col gap-2">
              <ModeCard
                ...
                title="Подбор фона"
                desc="Пиксели знака находятся автоматически; фон достраивается двухпроходно: продолжение градиентов + резкий структурный шов Telea. Справляется и с тонким, и с жирным текстом."
              />
              <ModeCard
            ...
            <label className="mt-3 block">
              ...
              <div className="mt-1.5 font-mono text-[10.5px] leading-snug text-mist-600">
                {mode === "dissolve"
                  ...
                  : radius <= 24
                    ? "Telea: структура фона из окрестности знака (лучше для тонкого текста)"
                    : `двухпроходный: фон по строкам/столбцам + резкий Telea-шов ${Math.min(40, Math.max(3, Math.round(radius * 0.5)))}px (лучше для жирного текста)`}
              </div>
            </label>
